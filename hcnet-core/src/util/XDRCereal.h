...
cereal_override(cereal::JSONOutputArchive& ar, const xdr::opaque_array<N>& s,
                const char* field)
{
    xdr::archive(ar, hcnet::binToHex(hcnet::ByteSlice(s.data(), s.size())),
                 field);
}
...
cereal_override(cereal::JSONOutputArchive& ar,
                const xdr::xarray<hcnet::Hash, N>& s, const char* field)
{
    std::vector<std::string> tmp;
    for (auto const& h : s)
    {
        tmp.emplace_back(
            hcnet::binToHex(hcnet::ByteSlice(h.data(), h.size())));
    }
...
void cereal_override(cereal::JSONOutputArchive& ar, const hcnet::PublicKey& s,
                     const char* field);

void cereal_override(cereal::JSONOutputArchive& ar,
                     const hcnet::MuxedAccount& muxedAccount,
                     const char* field);

void cereal_override(cereal::JSONOutputArchive& ar, const hcnet::Asset& s,
                     const char* field);
